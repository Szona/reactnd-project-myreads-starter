...
class Search extends Component {
  ...
  // Update the query state for searching purpose. If query is not empty,
  // call the API for results if not return an empty array to the state.
  // While typing, query result can be undefined or null, check the length
  ...
  // If the query is empty render an empty div to blank the screen between
  // multiple searches without closing Search component.
  // Search books results and filter them to check which one we
  // have already in our library. If we have it, we pass its information (title, shelf
  // id, authors, etc) to the render method, if not, pass the search books result for
  // the specific book.
  listBooks() {
